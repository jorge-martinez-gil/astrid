...
def compute_health_score(
    ...
    label_noise_rate = label_noise.get("suspected_label_noise_rate", None)
    label_noise_warn = label_noise.get("warning_threshold", 0.05)
    q_scores = [
...
def build_markdown_report(df, report, cfg_dict, file_name, file_bytes,
    ...
    label_noise = report.get("quality", {}).get("label_noise", {})
    label_noise_rate = label_noise.get("suspected_label_noise_rate")
    ...
    if label_noise_rate is not None:
        lines.append(f"- **Suspected label noise:** {float(label_noise_rate):.2%}\n")
    ...
    label_noise_candidates = label_noise.get("top_suspected_samples", [])
    if label_noise_candidates:
        lines.append("## Suspected Label-Noise Candidates\n")
        lines.append("| Sample | Observed | Suggested | Confidence | Noise Score |")
        lines.append("| --- | --- | --- | ---: | ---: |")
        for item in label_noise_candidates[:20]:
            sample = item.get("sample_id", item.get("row_index", ""))
            lines.append(
                f"| {sample} | {item.get('observed_label', '')} | "
                f"{item.get('suggested_label', '')} | "
                f"{float(item.get('suggested_confidence', 0)):.2%} | "
                f"{float(item.get('noise_score', 0)):.4f} |"
            )
        lines.append("")
...
def build_html_report(df, report, cfg_dict, file_name, file_bytes,
    ...
    label_noise = quality.get("label_noise", {})
    label_noise_rate = label_noise.get("suspected_label_noise_rate")
    ...
    label_noise_rows = "".join(
        "<tr>"
        f"<td>{esc(item.get('sample_id', item.get('row_index', '')))}</td>"
        f"<td>{esc(item.get('observed_label', ''))}</td>"
        f"<td>{esc(item.get('suggested_label', ''))}</td>"
        f"<td>{float(item.get('suggested_confidence', 0)):.2%}</td>"
        f"<td>{float(item.get('noise_score', 0)):.4f}</td>"
        "</tr>"
        for item in label_noise.get("top_suspected_samples", [])[:20]
    )
    label_noise_section = ""
    if label_noise_rate is not None:
        label_noise_section = (
            f"<h2>Suspected Label Noise</h2><p>Estimated rate: <strong>{float(label_noise_rate):.2%}</strong>. "
            "Candidates are model disagreements for human review, not confirmed errors.</p>"
        )
        if label_noise_rows:
            label_noise_section += (
                "<table><tr><th>Sample</th><th>Observed</th><th>Suggested</th>"
                f"<th>Confidence</th><th>Noise Score</th></tr>{label_noise_rows}</table>"
            )
    image_section = ""
    if is_image_report:
        def pct(value):
        ...
        image_rows = [
            ...
            ("Suspected label-noise rate", pct(label_noise_rate)),
