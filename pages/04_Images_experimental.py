...
with tab_quality:
    ...
    with col1:
        st.write("**Q1: Readability rate**")
        st.json(to_json_safe(q["readability"]))
    with col2:
        st.write("**Q3: Format conformance**")
        st.json(to_json_safe(q["format_conformance"]))

    col3, col4 = st.columns(2, gap="large")
    with col3:
        st.write("**Q2: Annotation linkage**")
        st.json(to_json_safe(q["annotation_linkage"]))
    with col4:
        st.write("**Q4: Metadata completeness**")
        st.json(to_json_safe(q["metadata_completeness"]))

    st.write("**Q5: Split coverage**")
    st.json(to_json_safe(q["split_coverage"]))

    st.write("**Q7: Bounding-box validity**")
    st.json(to_json_safe(q["bbox_validity"]))

    st.write("**Q9: Annotation schema consistency**")
    st.json(to_json_safe(q["annotation_schema_consistency"]))

    st.write("**Low resolution**")
    st.json(to_json_safe(q.get("low_resolution", {})))

    st.write("**Blur proxy**")
    st.json(to_json_safe(q.get("blur_proxy", {})))

    st.write("**Q12-Q13: Duplicates**")
    st.json(to_json_safe(q.get("duplicates", {})))

    st.write("**Q14: Cross-split leakage**")
    st.json(to_json_safe(q.get("cross_split_leakage", {})))

    st.write("**Q15: Conflicting duplicate labels**")
    st.json(to_json_safe(q.get("conflicting_duplicate_labels", {})))
...
with tab_reliability:
    ...
    st.write("**R1-R3: Annotator agreement**")
    st.json(to_json_safe(r.get("annotator_agreement", {})))

    st.write("**R10: Provenance coverage**")
    st.json(to_json_safe(r.get("provenance_coverage", {})))
...
with tab_robustness:
    ...
    st.write("**RB9: Outlier rate (MAD)**")
    st.json(to_json_safe(ro.get("image_feature_outliers_mad", {})))
...
with tab_transparency:
    ...
    st.write("**T1: Datasheet completeness**")
    ds = t_rep.get("datasheet_completeness", {})
    st.json(to_json_safe(ds))

    # T5: Traceability coverage
    st.write("**T5: Traceability coverage**")
    st.json(to_json_safe(t_rep.get("traceability_coverage", {})))

    # T8: Source attribution
    st.write("**T8: Source attribution coverage**")
    st.json(to_json_safe(t_rep.get("source_attribution_coverage", {})))

    # T11: Observability rate
    st.write("**T11: Observability rate**")
    st.json(to_json_safe(t_rep.get("observability_rate", {})))
...
with tab_security:
    ...
    st.write("**S1: Integrity**")
    st.json(to_json_safe(sec.get("integrity", {})))

    st.write("**S5: Suspicious samples**")
    st.json(to_json_safe(sec.get("suspicious_samples", {})))

    if "conflict_duplicate_rate" in sec:
        st.write("**S7: Conflict duplicate rate**")
        st.json(to_json_safe(sec["conflict_duplicate_rate"]))
